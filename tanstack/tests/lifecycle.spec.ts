...
/** TanStack Router renders one route tree, so this is a sanity check, not a filter. */
...
/** The Start build emits the client bundle as /assets/*.js next to /assets/*.css. */
const CLIENT_SCRIPTS = "**/assets/*.js";

test("No flash", async ({ page, browser, baseURL }) => {
  // Hold the client bundle back so the server-rendered document is on screen
  // long before hydration can start the lifecycle. Only the scripts: the
  // stylesheet lives in the same directory and holding that back would delay
  // layout rather than the lifecycle.
  await page.route(CLIENT_SCRIPTS, async (route) => {
  ...
  await blockedPage.route(CLIENT_SCRIPTS, (route) => route.abort());
...
test("Reduced motion", async ({ page, phases }) => {
  ...
  // Every phase still happens, and the end callback that resolves the blocker
...
test("TanStack: the blocker holds the URL until the outro ends", async ({ page, phases }) => {
  await page.goto("/");
  await expectSettledClean(page);

  await page.getByTestId("nav-work").click();

  // The blocker is the only hook that runs before the URL changes, so the whole
  // outro plays with the old address still in the bar and the old page still
  // mounted and laid out.
  await waitForPhase(page, "outro");
  expect(new URL(page.url()).pathname, "the push waits on the blocker").toBe("/");
  await expect(visiblePage(page).first()).toHaveAttribute("data-phase", "outro");

  // Only once the end state is reached does the blocker resolve and the router
  // push, load, and commit the destination.
  await page.waitForURL(atPath("/work"));
  await expectSettledClean(page);
  await onlyVisiblePage(page);

  // Every phase of the outgoing page, end included, was recorded against the
  // old path: the URL moved after the end state, not before it.
  expect(phasesFor(phases, "/")).toEqual(["initial", "intro", "settled", "outro", "end"]);
  expect(phasesFor(phases, "/work")).toEqual(["initial", "intro", "settled"]);
});
