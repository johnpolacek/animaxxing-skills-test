 ...
 *
 * Plain module, not client-only: TanStack Start renders every one of these
 * files on the server, and the pre-paint <ScriptOnce> needs the attribute name
 * from here while the server is rendering the document.
 */
...
/** Set by the controller once it is alive, so the failsafe stands down. */
...
/**
 * True when the pre-paint mark is gone, which means either reduced motion kept
 * it off or the failsafe released the page because hydration arrived too late.
 * Either way the content is already on screen, so hiding it again to play an
