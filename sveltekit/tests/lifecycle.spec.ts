...
/** SvelteKit shows one page tree, but assert it rather than assume it. */
...
test("No flash", async ({ page, browser, baseURL }) => {
  ...
  // lifecycle can start. Only the scripts: SvelteKit emits the stylesheet
  // under the same directory, and holding that back would delay layout rather
  // than the lifecycle.
  ...
  await blockedPage.route("**/_app/immutable/**/*.js", (route) => route.abort());
...
test("SvelteKit: a search-param-only change keeps the page settled", async ({ page, phases }) => {
  await page.goto("/");
  await expectSettledClean(page);

  // Same route, new search params: SvelteKit reuses the page component rather
  // than remounting it, so there is no lifecycle to replay. The page must stay
  // settled instead of outroing into a navigation that never re-enters it.
  await page.getByTestId("nav-filtered").click();
  await page.waitForURL((url) => url.pathname === "/" && url.search === "?q=1");

  // Long enough for an outro plus an intro to have shown up if either ran.
  await page.waitForTimeout(1000);
  expect(phasesFor(phases, "/"), "a search-param change replayed the lifecycle").toEqual([
    "initial",
    "intro",
    "settled",
  ]);
